...
- Vercel production 無法直連移民署 APIS 時，`/api/arrivals/realtime` 會回排程快照備援，讓公開 demo 仍能展示近即時資料形態；UI 會標示為「近即時快照」。
...
- 為了讓公開 demo 可展示近即時畫面，API 目前在 Vercel 直連失敗時回排程快照備援，response header 會帶 `X-Data-Fallback: snapshot`，`source.snapshot` 會是 `true`。
- `.github/workflows/update-realtime-snapshot.yml` 每小時執行一次 `scripts/update-realtime-snapshot.mjs`。如果 GitHub Actions runner 能連到移民署 APIS，就會更新 `api/arrivals/realtimeSnapshot.js` 並 push，Vercel 會自動重新部署。若 runner 也連不到 APIS，workflow 會保留舊快照、不覆蓋資料。
- `https://tourist-origin-radar.vercel.app/api/residence/annual` 正常回 200，年度資料 2002-2025 可用。

判斷：程式邏輯可用，問題在 Vercel 雲端出口到移民署 APIS 不通或被上游阻擋。公開 demo 的近即時畫面若顯示「近即時快照」，代表它正在展示排程快照，而不是 Vercel 當下直連上游。
