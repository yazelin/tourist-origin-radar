...
| 願望期望 | 實作 |
|----------|------|
| 即時資料抓取 | `/api/arrivals/realtime` 抓移民署近 3 小時入境預報（公開資料的即時上限）；年度資料每次載入即時抓觀光署 CSV；雲端連不上上游時退快照並明確標示 |
| 來源國比例圖表 | 來源國排行長條圖、市場佔比圓圖、2002-2025 多年趨勢圖、市場明細表、機場篩選 |
| 通知提醒 | 佔比/年增門檻可調，命中市場列於關鍵指標卡；opt-in 瀏覽器通知，命中變化時推播 |
| 資料匯出 | CSV / JSON 一鍵匯出（含佔比與年增欄位） |
...
- 提醒規則：來源佔比達門檻或年增達門檻，關鍵指標卡直接列出命中市場；可 opt-in 開啟瀏覽器通知，命中變化時推播。
- CSV / JSON 匯出（「來源國比例排行」面板右上）。
